...
for run = 1:6 % This averages together multiple runs to get the true estimate from GEMC
    ...
    All_data = dlmread(['MC_Type_AB_8_540_' int2str(run) '.txt']); % For the Monte Carlo sampling sets with Type B analysis (really Type AB)    
       ...
       swap = 0;
        ...
        while repeat == true
            ...
            repeat = false;
                ...
                liq_swap = [liq_swap; liq_dens(swap+1:last_step,j)]; % This concatenation need to be ";" not "," as I had in old codes
                vap_swap = vap_dens(equil_step:swap-1,j);
                vap_swap = [vap_swap; vap_dens(swap+1:last_step,j)]; % This concatenation needs to be ";" not "," as I had in old codes
